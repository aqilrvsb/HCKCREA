...
export default function LivehostTemplateBody() {
  ...
  const prompt =
    `Animate the character to follow ONLY the reference video's hand motion, frame for frame. Preserve the character's exact appearance from the reference image — face, hair or head covering, skin tone, and full outfit — with maximum fidelity. Do not change the appearance, clothing, colors, or face. ONLY the hands, fingers, and forearms move, mirroring the reference; everything else stays completely still and rigid — the head, face, torso, and shoulders barely move. Critically, ALL clothing is locked and static: the hijab/headscarf, baju/shirt, and any fabric must NOT move, billow, flutter, ripple, sway, stretch, or shift its folds at all — treat the outfit and hijab as a frozen, rigid, non-deforming surface with absolutely zero fabric motion. Photorealistic, ultra-detailed, natural realistic hands, clean consistent studio lighting. Keep the background exactly as in the reference image — a solid, flat, uniform ${colorWord} chroma-key screen; do not add any scenery, objects, or shadows and do not change the background color. Locked static camera — no zoom, no pan, no camera shake. Keep the character centered with consistent framing throughout.`;
