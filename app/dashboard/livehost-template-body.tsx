...
export default function LivehostTemplateBody() {
  ...
  const prompt =
    `Animate ONLY the character's hands to follow the reference video's hand motion, frame for frame. Everything else is completely frozen. Preserve the character's exact appearance from the reference image — face, hair or head covering, skin tone, and full outfit — with maximum fidelity; do not change the appearance, clothing, colors, or face. The head, face, and every facial feature are LOCKED and motionless: the eyes, eyebrows, mouth, lips, jaw, and expression do NOT move at all — no talking, no mouth movement, no smiling change, no blinking change, no head turn, tilt, or nod. The torso, shoulders, and neck stay perfectly still and upright. ALL clothing is locked and static: the hijab/headscarf, baju/shirt, and any fabric must NOT move, billow, flutter, ripple, sway, stretch, or shift its folds — treat the outfit and hijab as a frozen, rigid, non-deforming surface with absolutely zero fabric motion. ONLY the hands, fingers, and forearms move, mirroring the reference. Photorealistic, ultra-detailed, natural realistic hands, clean consistent studio lighting. Keep the background exactly as in the reference image — a solid, flat, uniform ${colorWord} chroma-key screen; do not add any scenery, objects, or shadows and do not change the background color. Locked static camera — no zoom, no pan, no camera shake. Keep the character centered with consistent framing throughout.`;
