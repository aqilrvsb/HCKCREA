...
export default function LivehostTemplateBody() {
  ...
  const prompt =
    "Animate the character to follow the reference video's motion exactly, frame for frame. Preserve the character's exact appearance from the reference image — face, hair or head covering, skin tone, and full outfit — with maximum fidelity. Do not change the appearance, clothing, colors, or face. Keep the torso and shoulders stable and upright; only the hands and arms move as in the reference. Photorealistic, ultra-detailed, natural realistic hands. Place the character against a solid, uniform, flat pure chroma-key green screen background (hex #00FF00) — perfectly even, no gradient, no texture, no shadow cast on the background, no props or scenery — so the background can be removed cleanly later. Even, soft studio lighting on the character only; keep the green background evenly lit and free of spill. Locked static camera — no zoom, no pan, no camera shake. Keep the character centered with consistent framing throughout.";
