...
export default function AutoUgcTab({ projectId }: { projectId?: string } = {}) {
  ...
  async function submit() {
    ...
    try {
      ...
      const body: any = {
        ...
        quantity,
        // Auto UGC: Grok Imagine 1.5 only, duration slider up to 30s.
  ...
  async function approveVerifyPlan() {
    ...
    abortRef.current = new AbortController();
    try {
      const r = await fetch("/api/generate/auto-ugc/approve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...pendingPayload,
  ...
  return (
    <div className="rounded-3xl p-6 md:p-8 space-y-5" style={sectionBg}>
      <Card borderColor={AMBER}>
        ...
        >
          {/* Auto UGC: choose to GENERATE a new avatar (persona below) or
              use an EXISTING avatar the client uploads. Existing avatar is
              kept consistent across every video. */}
          <div className="flex gap-2 mb-3">
            <DurationBtn active={avatarMode === "create"} onClick={() => setAvatarMode("create")}>
              ✨ Jana Avatar
            </DurationBtn>
            <DurationBtn active={avatarMode === "existing"} onClick={() => setAvatarMode("existing")}>
              📤 Avatar Sedia Ada
            </DurationBtn>
          </div>
            ...
            <div className="flex items-center gap-3 mb-3">
              {avatarUrl ? (
                ...
                <label
                  ...
                  style={{ border: `1px dashed ${AMBER}`, color: AMBER }}
                >
                  {uploadingAvatar ? (
              ...
              <p className="text-[11px] text-[var(--color-text-muted)]">
                Avatar ni akan digunakan (konsisten) untuk semua video. Baju & scene
          ...
          {/* Persona selectors — used when generating a new avatar. Hidden
              when an existing avatar is uploaded (identity comes from the
              photo). */}
          {avatarMode === "create" && (
        ...
        {/* Provider — Auto UGC is Grok Imagine 1.5 ONLY (Veo / Sora 2 /
            GeminiOmni removed). Duration is a slider up to 30s; 16–30s
            auto-splits into 2 balanced segments (Seg 1 / Seg 2). */}
        <label className="block text-xs font-mono uppercase tracking-widest text-[var(--color-text-muted)] font-bold mb-2">
          Provider
        </label>
        <div className="flex gap-2 mb-3">
          <DurationBtn active onClick={() => {}}>
            ⚡ Grok Imagine 1.5
          </DurationBtn>
        </div>
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs font-mono uppercase tracking-widest text-[var(--color-text-muted)] font-bold">
              Durasi: {ugcDuration}s → {splitLabel(ugcDuration)}
            </label>
            {grokRate != null && (
              <span className="text-xs font-bold" style={{ color: "var(--color-orange)" }}>
                ~RM{((grokRate ?? 0) * ugcDuration).toFixed(2)} / video
              </span>
            )}
          </div>
