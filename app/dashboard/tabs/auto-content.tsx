...
type ManualProduct = {
  ...
  begKuningUrl?: string; // optional Beg Kuning link. Present → saves as a
                         // "Beg Kuning Product"; empty → "Tiada Link Product".
...
export default function AutoContentTab({ projectId }: { projectId?: string } = {}) {
  ...
  // Reconstruct the Beg Kuning link from a saved product_id (TikTok Shop).
  ...
  async function saveProduct(i: number) {
    ...
    try {
      ...
      // Reload both lists so a bucket move reflects immediately.
      loadSavedManual();
      loadSavedAffiliate();
    ...
    <div className="rounded-3xl p-6 md:p-8 space-y-5" style={sectionBg}>
      <Card borderColor={AMBER}>
        ...
        {/* 🔗 Beg Kuning Product dropdown — saved products WITH a link.
            Pick → reload Name + Detail + attachments + the Beg Kuning link. */}
        {showSavedAffiliate && (
...
function ManualProductCard({
  ...
  // Save rules (unified): Name + Detail + 3 attachments. The Beg Kuning link
  // is optional — its presence just decides the bucket at save time.
  const canSave =
    imgCount >= 3 &&
    !!(product.name || "").trim() &&
    !!(product.detail || "").trim();
      ...
      {mode === "manual" ? (
        <>
          ...
          <textarea
            ...
            style={{ background: "#ffffff", border: "1px solid #e8e0d8", color: "#1a1a1a" }}
          />
          {/* Link Beg Kuning (optional) — fill it + Save → the product is
              saved as a Beg Kuning Product; leave empty → Tiada Link Product. */}
          <label className="block text-[9px] font-bold uppercase tracking-wider mb-1" style={{ color: "#888" }}>
