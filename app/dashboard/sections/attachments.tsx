...
export default function AttachmentsSection({
  ...
  const presetCounts = {
    all: presets.length,
    product: presets.filter((p) => p.category === "product").length,
      ...
      {shownPresets.length > 0 && (
        <div className="mb-6">
          <div className="text-[11px] font-bold uppercase tracking-wider mb-2" style={{ color: "var(--color-text-muted)" }}>
            Default — tersedia untuk semua (tidak boleh padam)
          </div>
            ...
            {shownPresets.map((p) => (
              <div key={p.id} className="relative rounded-lg overflow-hidden border" style={{ borderColor: "var(--color-border)", background: "var(--color-surface)" }}>
                <div className="block w-full aspect-square bg-black/40 relative">
                  ...
                  <img src={p.public_url} alt={p.name} className="w-full h-full object-cover" loading="lazy" />
                  <span className="absolute top-1.5 left-1.5 inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider"
                    style={{ background: "rgba(99,102,241,0.85)", color: "white" }}>
                    {p.category === "avatar" ? <UserCircle2 className="w-3 h-3" /> : <Package className="w-3 h-3" />}
                    Default
                  </span>
                </div>
