...
export async function GET(req: Request) {
  ...
  // Storytelling rows used to be auto-synthesized into Storage from the
  // history table (so they'd appear without a manual Save click). Reverted
  // by user request: storytelling rows now live in history with a TTL
  // countdown badge and only graduate to Storage when the user explicitly
  // clicks Save. That makes Storage a "things I chose to keep" list
  // instead of dumping every generation into it.
