...
const { data: pool } = await sb.from("livehost_pool").select("endpoint_id,status").order("created_at");
for (const p of pool) {
  ...
  const e = d?.endpoint;
  const wc = (e?.workers || []).length;
  console.log(`${p.endpoint_id}  db=${p.status}  workers=${wc}  minNum=${e?.workerConfig?.minNum}  freeTimeout=${e?.workerConfig?.freeTimeout}  image=${(e?.image?.image||"").split(":").pop()}`);
}
